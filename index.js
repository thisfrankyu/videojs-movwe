...
function loadLibrary() {
    socket.emit('library', {});
    socket.on('libraryResults', this.loadLibraryMenu.bind(this, 0));
}

function loadLibraryMenu(offsetCount, jsonResult) {
    var libraryMenu = $('#libraryMenu');
    var offset = '&nbsp;'.repeat(offsetCount);

    Object.keys(jsonResult).forEach((entry) => {
        if (_.isString(jsonResult[entry])) {
            libraryMenu.append(`<option class="libraryFile" id=${jsonResult[entry]} value="${entry}">${offset + entry}</option>`);
        } else {
            libraryMenu.append(`<option class="libraryDirectory" value="${entry}" disabled>${offset + entry}</option>`);
            this.loadLibraryMenu(offsetCount + 4, jsonResult[entry]);
        }
    });
}
...
function loadVideo(event) {
    ...
    socket.emit('auth', {}, function(token){
        ...
        videoPlayer.src({
            type: "video/webm",
            src: new PlexUrl(address, metadata, port, authToken, 'plexurl', {videoResolution: resolution, offset: offset})
        });
        var metadataId;
        for (metadataId = 3; metadataId < 20; metadataId++){
            $.ajax({
                type: "GET",
                url: "http://"+address+":"+port+"/clientLibrary/metadata/"+metadataId+"?X-Plex-Token="+authToken,
                dataType: "xml",
